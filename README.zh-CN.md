...
`model` 字段用来选择预设，合法格式只有 `preset_id` 和 `preset_id/session_id`。
...
- `default` 表示运行 `config/presets/default.yaml`
...
v1 API 的 session history 只保存在当前进程内存中，服务重启后会丢失。
...
如果 `model` 为空或预设名不存在，则回退到 `default_preset`。如果 `model` 格式非法，则不会回退，而是直接返回 `invalid_model`。
...
变量引用使用 `{block_tag.task_tag.result}` 语法，只能引用前面已执行 Block 的结果，同一 Block 内的 Task 并行执行、结果不可互相引用。Chat 消息里还可以使用 `{block_tag.task_tag.history[index]}` 读取保留的 session history，其中 `[0]` 是当前保留记录中最新的一条，`[-1]` 是最旧的一条。
